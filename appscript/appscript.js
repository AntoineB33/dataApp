...
function onEdit(e) {
  ...
  for(var i = 1; i<nbLineBef; i++) {
    perRef.push(-1);
    if(!values[i].some((val)=>val.length!=0)) {
      data.push(-1);
      continue;
    }
    ...
    for(var k = 0; k<values[i][0].length; k++) {
      ...
      if(!stop && stat>-1) {
        if(perRef[i]>-1) {
          if(!periods[perRef[i]][3].some(function(e) {return e.toLowerCase()==val;})) {
            periods[perRef[i]][3].push(elem);
          }
        } else {
